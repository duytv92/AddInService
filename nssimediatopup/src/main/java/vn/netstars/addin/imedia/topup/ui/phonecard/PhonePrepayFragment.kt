...
import kotlinx.android.synthetic.main._nss_fragment_phone_prepay.*
...
import vn.netstars.addin.imedia.topup.ui.infocard.CardInfoFragment
...
class PhonePrepayFragment private constructor(): BaseFragment<PhoneCardViewModel>() {

    companion object {
        private const val LIST_CARD_BRANDS_KEY = "LIST_CARD_BRANDS_KEY"
        fun newInstance(listDataCard: ArrayList<CardBrandModel>): PhonePrepayFragment {
            val args = bundleOf(
                LIST_CARD_BRANDS_KEY to listDataCard
            )
            val fragment = PhonePrepayFragment()
    ...
    override fun getLayoutResId(): Int = R.layout._nss_fragment_phone_prepay
    ...
    override fun initView() {
        ...
        btn_pay.setSafeOnClickListener {
            val listItem = mShoppingCartAdapter.data
            val listIemChoose = ArrayList<CardItem>()

            listItem.forEach {
                listIemChoose.add(CardItem(it.cardType, it.cardStringValue))
            }
            nssNavigateToFragment(CardInfoFragment.newInstance(listIemChoose))
        }
